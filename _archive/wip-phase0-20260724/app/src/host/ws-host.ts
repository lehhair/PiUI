...
export class WsHostTransport {
  ...
  async connect(): Promise<void> {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      ws.onclose = () => {
        this.setStatus("closed")
        this.ws = null
        for (const [, p] of this.pending) p.reject(new Error("host disconnected"))
        this.pending.clear()
      }
      ws.onmessage = ev => {
        let msg: WireMessage
        ...
        if (msg.channel === "event") this.emit(msg.event)
  ...
  async request(cmd: HostCommand): Promise<HostResponse> {
    if (!this.ws || this.status !== "open") await this.connect()
    const ws = this.ws
    if (!ws || this.status !== "open") return { ok: false, error: "not connected" }
    const id = cmd.id ?? `c${++this.seq}`
    return new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        this.pending.delete(id)
        resolve({ ok: false, error: `timeout ${cmd.type}` })
      }, 120_000)
      this.pending.set(id, {
        resolve: r => {
          clearTimeout(timer)
      ...
      ws.send(JSON.stringify({ channel: "request", id, cmd: { ...cmd, id } }))
    })
  }
...
let singleton: WsHostTransport | null = null

export function getPiHost(): WsHostTransport {
  if (!singleton) singleton = new WsHostTransport()
  return singleton
}

export async function ensurePiHostConnected(): Promise<WsHostTransport> {
  const h = getPiHost()
  await h.connect()
  return h
}
