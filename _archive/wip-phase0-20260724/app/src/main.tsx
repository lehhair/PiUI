...
async function connectPiHost() {
  try {
    const { ensurePiHostConnected } = await import('./host/ws-host')
    await ensurePiHostConnected()
    console.info('[PiUI] Pi Host connected (ws://127.0.0.1:8787)')
  } catch (err) {
    console.warn('[PiUI] Pi Host not connected — run npm run dev:host', err)
  }
}
...
function startApp() {
  ...
  // 浏览器开发：不启 OpenCode serve，改连 Pi Host
  // Tauri 桌面以后再接 sidecar
  if (!isNativeTauri) {
    void connectPiHost()
  } else {
